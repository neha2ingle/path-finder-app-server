/*!
 * File taken from lowdb-api package and updated to resolve error when path with prefix is used.
 * Fix applied at line number 113
 * lowdb-api
...
function lowdbApi(file, options = {}) {
  ...
  return function lowdbApi(req, res) {
    ...
    if (options.prefix && pathname.indexOf(options.prefix) === 0) {
      ...
      // FIX
      pathname = pathname.substr(options.prefix.length);
    }
